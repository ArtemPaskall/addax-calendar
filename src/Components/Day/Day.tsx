...
function Day({ dayInfo }: { dayInfo: DayInfo }) {
  ...
  const addTaskHandler = (dayInfo: DayInfo) => {
    ...
    setTasksList(prevTasksList => [
      ...prevTasksList.filter(task => task.date !== dayInfo.fullDate),
      newTask,
    ])
    setTaskText('')
  }
  ...
  const dropHandler = (e: React.DragEvent<HTMLLIElement>) => {
    ...
    const currentTask = tasksList
      .find(task => task.date === currentDay?.fullDate)
  ...
  return (
    <>
      <li
        className={`${dayInfo.inactive ? 'inactive' : ''} ${
          dayInfo.active ? 'active' : ''
        } day-cell`}
        draggable={true}
      ...
      >
        ...
        {tasksList
          .find(task => task.date === dayInfo.fullDate)
          ?.tasks.filter(task =>
            searchByText
              ? task.text.toLocaleLowerCase().includes(searchByText.toLocaleLowerCase())
              : true,
          )
          .filter(task => (searchByLabel ? task.labels.includes(searchByLabel) : true))
          .map(task => (
